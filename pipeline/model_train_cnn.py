...
# ==== 실험 옵션 ====
SPACE_VERSION   = "v8_cnn"   # ← 기존과 겹치지 않게 버전 업데이트
USE_EXISTING    = True
OOF_TRAIN_FRAC  = 0.7
OOF_VAL_BLOCKS  = 3
...
def evaluate_on_holdout(saved):
    ...
    _, df_holdout = _train_holdout_split(df_all)  # 뒤쪽 구간만 홀드아웃
    # --- 번들 로드 (스케일러/피처/보정기/베스트 파라미터) ---
    ...
    # --- 예측 ---
    _, p, _ = evaluate_model(cnn, dl)  # p: 확률 벡터
    ...
    # --- 응급 가드: 폴라리티 역전 시 확률 뒤집기 ---
    auc_try = roc_auc_score(vt, p_c) if len(np.unique(vt)) > 1 else 0.5
    if auc_try < 0.5:
        logging.warning(f"[HOLDOUT] AUC<0.5 detected (raw={auc_try:.4f}) → flipping probabilities")
        p_c = 1.0 - p_c
        auc_try = roc_auc_score(vt, p_c) if len(np.unique(vt)) > 1 else 0.5

    # --- 최종 임계값 적용 ---
    t    = float(best_full.get("best_threshold", 0.5))
    ...
    # 민감도/특이도 (TPR/TNR)
    tn, fp, fn, tp = confusion_matrix(vt, pred, labels=[0,1]).ravel()
    ...
    logging.info(
        f"[HOLDOUT] AUC_full={auc_full:.4f} | ACC={acc:.4f} F1={f1:.4f} "
        f"| Sens={sensitivity:.4f} Spec={specificity:.4f} AUPR={aupr:.4f} | "
