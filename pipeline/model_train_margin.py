...
# ==== 실험 옵션 ====
SPACE_VERSION   = "v8_band_gated"   # ← 기존과 겹치지 않게 버전 업데이트
USE_EXISTING    = True
OOF_TRAIN_FRAC  = 0.6
OOF_VAL_BLOCKS  = 3
...
def evaluate_holdout(
    ...
    p_hold,
    *,
    calibrator=None,            # ProbabilityCalibrator 또는 None
    ...
    """
    평가 파이프라인(정렬판):
    1) flip-guard → 2) 확률 보정(transform) → 3) margin/coverage → 4) 지표/score
    반환: (metrics_dict, score_float)
    """
    if logger is None:
        class _N: 
    ...
    # 1) flip-guard (AUC<0.5면 1-p 뒤집기)
    p_fixed, auc_pre, flipped = apply_flip_guard(y_hold, p_hold)
    logger.info(f"[HOLDOUT] pre_auc={auc_pre:.4f} flipped={flipped}")

    # 2) 확률 보정(보정기가 있으면 transform)
    if calibrator is not None:
        p_cal = np.asarray(calibrator.transform(p_fixed), float)
    else:
        p_cal = np.asarray(p_fixed, float)
    p_cal = np.clip(p_cal, 1e-6, 1-1e-6)

    # 3) margin/coverage (보정된 확률에 적용 必)
    if reject_margin > 0:
        mask = (np.abs(p_cal - 0.5) >= float(reject_margin))
    else:
        mask = np.ones_like(p_cal, dtype=bool)
    coverage = float(mask.mean())

    # 4) 전체 지표
    try:
        auc_full = roc_auc_score(y_hold, p_cal)
    except Exception:
        auc_full = 0.5
    yhat_full = (p_cal >= 0.5).astype(int)
    acc_full  = accuracy_score(y_hold, yhat_full)
    f1_full   = f1_score(y_hold, yhat_full) if len(np.unique(y_hold))>1 else 0.0
    ...
    logger.info(
        "[HOLDOUT] auc_full=%.4f acc=%.4f f1=%.4f | margin=%.3f coverage=%.3f | "
        "auc_bin=%s acc_bin=%s f1_bin=%s | brier=%.4f ece=%.4f score=%.4f",
        float(auc_full), float(acc_full), float(f1_full),
        float(reject_margin), coverage,
...
def run_optuna():
    ...
    study.optimize(objective, n_trials=2000, timeout=3600)  # ▼ 5000 → 800
...
def retrain_and_save(study):
    ...
    return {
        ...
        "low_X_seq": low_X_seq, "high_X_seq": high_X_seq,
        "yv": yv, "ctx_val": None,
        "cal_low": cal_low, "cal_high": cal_high
    }
...
def evaluate_on_holdout(saved):
    df_all = pd.read_csv(enhanced_csv, parse_dates=["date"]).sort_values("date").reset_index(drop=True)
    _, df_holdout = _train_holdout_split(df_all)

    bundle = joblib.load(Path(saved["saved_dir"]) / "preproc_and_calibrators.pkl")
    low_scaler = bundle["low_scaler"]; high_scaler = bundle["high_scaler"]
    low_feats  = bundle["low_feats"];  high_feats  = bundle["high_feats"]
    cal_low    = bundle["cal_low"];    cal_high    = bundle["cal_high"]
    ...
    trans = WaveAttTransformerClassifier(
        ...
        n_scales=int(best_full.get("n_scales",3)), dtw_gamma=0.1
    )
    cnn = CNN1DClassifier(in_channels=len(high_feats),
                          hidden=int(best_full.get("cnn_hidden",128)),
                          dropout=float(best_full.get("cnn_dropout",0.2)))
    trans.load_state_dict(torch.load(Path(saved["saved_dir"])/"transformer_low_final.pt", map_location=DEVICE))
    cnn.load_state_dict(torch.load(Path(saved["saved_dir"])/"cnn_high_final.pt", map_location=DEVICE))
    trans.to(DEVICE).eval(); cnn.to(DEVICE).eval()

    xgb_meta = joblib.load(Path(saved["saved_dir"])/"meta_xgb.pkl")
    lr_meta_path = Path(saved["saved_dir"])/"meta_lr.pkl"
    lr_meta = joblib.load(lr_meta_path) if lr_meta_path.exists() else None
    w = {"w_x":1.0, "w_l":0.0}
    w_path = Path(saved["saved_dir"])/"meta_blend_weights.pkl"
    if w_path.exists(): w = joblib.load(w_path)
    w_x, w_l = float(w.get("w_x",1.0)), float(w.get("w_l",0.0))

    dfh = df_holdout.reset_index(drop=True)
    X_low  = low_scaler.transform(dfh[low_feats].fillna(0).values.astype(float))  if len(low_feats)>0  else np.zeros((len(dfh),0))
    X_high = high_scaler.transform(dfh[high_feats].fillna(0).values.astype(float)) if len(high_feats)>0 else np.zeros((len(dfh),0))
    y      = dfh["label_up"].values

    seq_len = int(best_full["seq_len"])
    dl_low  = DataLoader(SequenceDataset(X_low, y, seq_len, False), batch_size=128, shuffle=False)
    dl_high = DataLoader(SequenceDataset(X_high,y, seq_len, False), batch_size=128, shuffle=False)
    _, p_low, _  = evaluate_model(trans, dl_low)
    _, p_high, _ = evaluate_model(cnn,  dl_high)

    L = min(len(p_low), len(p_high), len(y)-seq_len)
    ...
    lpv_c = cal_low.transform(p_low[:L])  if getattr(cal_low, "trained", False)  else p_low[:L]
    hpv_c = cal_high.transform(p_high[:L]) if getattr(cal_high,"trained", False) else p_high[:L]

    ctx_cols = best_full.get("meta_ctx_cols", [])
    ctx_val_df = dfh.iloc[seq_len:seq_len+L, :][ctx_cols].fillna(0) if ctx_cols else None
    p_blend = _make_regime_blend_series(ctx_val_df, lpv_c, hpv_c)
    X_val = _build_meta_matrix_from_probs(
        lpv_c, hpv_c,
        (ctx_val_df.values if ctx_val_df is not None else None),
    ...
    auc = roc_auc_score(vt, p) if len(np.unique(vt))>1 else 0.5
