...
def duck_upsert_by_name(table: str, df: pd.DataFrame, key_cols=("date",), schema: str = "bronze"):
    ...
    ✅ FIX: pandas datetime 컬럼 ↔ DuckDB 컬럼 타입 자동 정렬(ALTER … TYPE TIMESTAMP)
    """
    import duckdb
    if df is None or df.empty:
        print(f"[DuckDB] skip {schema}.{table}: empty"); return

    # pandas dtype 기준 사전 수집
    datetime_cols = [c for c in df.columns if pd.api.types.is_datetime64_any_dtype(df[c])]
    # (timestamp로 저장되길 원하는 문자열/ISO8601 컬럼이 있다면 여기서 선변환)
    # 예: pubDate/fetchedAt이 문자열인 채로 들어오면 to_datetime으로 표준화
    for c in ("pubDate", "fetchedAt"):
    ...
    # 1) 스키마 정렬: datetime 컬럼은 TIMESTAMP로 강제
    #    (기존에 INTEGER 등으로 만들어져 있던 pubDate/fetchedAt을 교정)
    for col in datetime_cols:
        try:
            con.execute(f"ALTER TABLE {schema}.{table} ALTER COLUMN {col} TYPE TIMESTAMP;")
        except Exception as e:
            # 존재하지 않거나 이미 TIMESTAMP면 무시
            print(f"[DuckDB][ALTER][WARN] {schema}.{table}.{col} -> TIMESTAMP: {e}")

    # 2) 키 기준 삭제 후 BY NAME 삽입
    cond = " AND ".join([f"t.{k} = s.{k}" for k in key_cols if k in df.columns])
    if cond:
...
def run_for_date(target_day: date_cls, query="코스피", topic_model=None):
    ...
    # ✅ 시간 컬럼 정규화: pubDate / fetchedAt → UTC 기준 tz-naive TIMESTAMP
    #   - pubDate: 기사 발행 시각 (원문이 tz포함 ISO8601/문자열/None 섞여있음)
    #   - fetchedAt: 수집 시각 (기존 STAMP_UTC()는 '...Z' 문자열 → TIMESTAMP로 변환)
    for c in ("pubDate", "fetchedAt"):
        if c in raw.columns:
            raw[c] = (
                pd.to_datetime(raw[c], utc=True, errors="coerce")
                  .dt.tz_convert(None)
            )
    ...
    duck_upsert_by_name(
        "raw_news",
        raw.drop(columns=["date"], errors="ignore"),
        key_cols=("url",),
        schema="bronze"
    )
