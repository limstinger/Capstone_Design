...
if __name__ == '__main__':
    ...
    model = FusionLSTMPerformer(
        ...
        heads        = params['heads'],        # ← 추가 필요
        depth        = params['depth'],        # ← 추가 필요
        max_seq_len  = params['seq_len']       # 또는 따로 지정
    ).to(device)
    ...
    torch.save(model.state_dict(), root / 'models' / 'fusion_model_lineared_0613.pth')
    print("✅ 최종 모델 (binary classifier) 학습 및 저장 완료")

    # (5) 최적 파라미터 JSON으로 저장
    with open(root / 'training' / 'modeling' / 'best_params_lineared.json', 'w') as f:
