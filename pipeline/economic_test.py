# -*- coding: utf-8 -*-
"""
KRX/FX/XAU/WTI + 기준금리(한국은행) 수집 파이프라인
- lake/bronze/ 아래 파케(partitioned by date) 저장
- data_analyze/economic_indicator/krw_fed_rate_announcements.csv 생성(덮어쓰기)
  컬럼: date, decision_rate, prev_rate
- RAW CSV 업서트(누적) 형식
  ① 환율/지수: 'Date','종가' ② 금/WTI 묶음: 'WTI_USD','XAU_USD','WTI_KRW','XAU_KRW'
- DuckDB 테이블 MERGE 업서트 (schema=bronze)
- 단일 날짜 실행 + 날짜 범위 실행(--start --end) 지원
"""
...
def write_partition(df: pd.DataFrame, dataset: str, date_col="date", mode: str = "skip_if_exists"):
    ...
    if df[date_col].dropna().empty:
        print(f"[SKIP] {dataset}: all {date_col} are NaT")
        return
    ...
    if mode == "skip_if_exists" and path.exists():
        print(f"[SKIP] {dataset}: {path.name} already exists")
        return
...
def duck_upsert(table: str, df: pd.DataFrame, key_cols=("date",), schema: str = "bronze"):
    import duckdb
    if df is None or df.empty:
...
# 2) XAUUSD (Stooq)
# =============================================================================
def fetch_xauusd_today_stooq(target_day: date_cls = TODAY, strict_today=True, backfill_days=3) -> pd.DataFrame:
    ...
    m = df[df["Date"].dt.date == target_day]
    if not m.empty:
...
# =============================================================================
# 6) VKOSPI — KRX 데이터포털(getJsonData.cmd)
# =============================================================================
...
def _krx_session():
    ...
    s.get(KRX_REF, timeout=30)  # 세션/쿠키 확보
    return s
...
def fetch_vkospi_krx_range(start_ymd: str, end_ymd: str) -> pd.DataFrame:
    ...
    close_col = ("IDX_CLSPRC" if "IDX_CLSPRC" in df.columns else
                 "TDD_CLSPRC" if "TDD_CLSPRC" in df.columns else
                 next((c for c in df.columns if "CLSPRC" in c), None))
...
# 하루 처리 함수
# =============================================================================
def process_one_day(TARGET_DAY: date_cls):
    TARGET_YMD = TARGET_DAY.strftime("%Y%m%d")
    print(f"\n>>> TARGET_DAY={TARGET_DAY} (KST) TARGET_YMD={TARGET_YMD}")

    # (a) KOSPI
    kospi = fetch_kospi_latest_yahoo_on_or_before(TARGET_DAY, days_window=14)
    ...
    # (b) VKOSPI
    vkospi = fetch_vkospi_latest_on_or_before_krx(TARGET_YMD, lookback_days=30)
    ...
    # (c) FX
    fx_today = fetch_ecos_fx_T_or_Tminus1(TARGET_YMD, max_back_days=0)  # RAW용 타깃날
    fx_all = fx_today if not fx_today.empty else fetch_ecos_fx_T_or_Tminus1(TARGET_YMD, max_back_days=7)  # 파케/DB 최신가
    ...
    # (d) XAU
    xau_today = fetch_xauusd_today_stooq(TARGET_DAY, strict_today=True, backfill_days=0)
    xau_all = xau_today if not xau_today.empty else fetch_xauusd_today_stooq(TARGET_DAY, strict_today=False, backfill_days=3)
    ...
    # (e) WTI — EOD(T-1)
    wti = choose_wti_for_training(TARGET_DAY, stale_threshold_days=2)
    eod = wti["eod"]
    ...
    # (f) 기준금리
    base_today_or_recent = fetch_base_rate_T_or_Tminus1(TARGET_YMD, max_back_days=7)
    ...
    features = {
        "asof_date_kst": TARGET_DAY.isoformat(),
    ...
    # (i) RAW CSV — 타깃 날짜 행만
    if not fx_today.empty and fx_today["date"].iloc[-1].date() == TARGET_DAY:
        ...
        print("[RAW] FX 타깃 날짜 값 없음 → RAW CSV 스킵")

    if not xau_today.empty and xau_today["date"].iloc[-1].date() == TARGET_DAY:
        _upsert_price_csv(RAW / "XAUUSD Historical Data.csv", xau_today["date"].iloc[-1], xau_today["xau_usd"].iloc[-1])
    else:
        print("[RAW] XAU 타깃 날짜 값 없음 → RAW CSV 스킵")

    if not kospi.empty and pd.to_datetime(kospi["date"].iloc[-1]).date() == TARGET_DAY:
        _upsert_price_csv(RAW / "KOSPI Historical Data.csv", kospi["date"].iloc[-1], kospi["kospi"].iloc[-1])
    else:
        print("[RAW] KOSPI 타깃 날짜 종가 없음/휴장 → RAW CSV 스킵")

    if not vkospi.empty and pd.to_datetime(vkospi["date"].iloc[-1]).date() == TARGET_DAY:
        _upsert_price_csv(RAW / "KOSPI Volatility Historical Data.csv", vkospi["date"].iloc[-1], vkospi["vkospi"].iloc[-1])
    else:
        print("[RAW] VKOSPI 타깃 날짜 값 없음 → RAW CSV 스킵")

    usd_krw_for_raw = (float(fx_today["usd_krw"].iloc[-1]) if not fx_today.empty and "usd_krw" in fx_today.columns else None)
    xau_usd_for_raw = (float(xau_today["xau_usd"].iloc[-1]) if not xau_today.empty and "xau_usd" in xau_today.columns else None)
    if usd_krw_for_raw is not None and xau_usd_for_raw is not None:
        row = {
            "WTI_USD": None,  # EOD(T-1)이므로 TARGET_DAY 행엔 기록하지 않음
        ...
        print("[RAW] gold_wti_prices_krw 타깃 날짜 계산 불가 → 스킵")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--yesterday", action="store_true", help="오늘 대신 전날(T-1) 데이터를 저장")
    ap.add_argument("--target", type=str, default=None, help="저장할 기준 날짜 (YYYY-MM-DD). 예: --target 2025-09-24")
    ap.add_argument("--start", type=str, default=None, help="범위 시작일 (YYYY-MM-DD)")
    ap.add_argument("--end", type=str, default=None, help="범위 종료일 (YYYY-MM-DD, 포함)")
    args = ap.parse_args()
    ...
    if args.start and args.end:
        ...
        # 범위 끝 기준으로 기준금리 공시 CSV 1회 생성
        save_base_rate_announcements_csv(ANNOUNCE_CSV, start_ymd="20000101", end_ymd=end_d.strftime("%Y%m%d"))
    else:
        # 단일 날짜 실행 (기존 동작 유지)
        TARGET_DAY = (datetime.now(KST).date() - timedelta(days=1)) if args.yesterday else (
            (datetime.strptime(args.target, "%Y-%m-%d").date() if args.target else datetime.now(KST).date())
        )
        process_one_day(TARGET_DAY)
        save_base_rate_announcements_csv(ANNOUNCE_CSV, start_ymd="20000101", end_ymd=TARGET_DAY.strftime("%Y%m%d"))
