...
from sklearn.metrics import roc_auc_score, accuracy_score, f1_score, average_precision_score, recall_score, confusion_matrix, classification_report
...
FAST_WAVELET = True   # True면 훈련 중 SWT 미사용(빠른 경로)
USE_DTW      = False  # True면 Soft-DTW 사용, False면 패스스루
...
# ==== 실험 옵션 ====
SPACE_VERSION   = "v6_waveatt_log"   # ← 기존과 겹치지 않게 버전 업데이트
USE_EXISTING    = True
OOF_TRAIN_FRAC  = 0.6
OOF_VAL_BLOCKS  = 3
...
USE_LOGRET_FILTER       = True          # True면 학습 전 보합(변동 거의 없음) 샘플 제거
LOGRET_ABS_TAU          = 0.0005        # 절대 임계치: |logret| < 0.05% 제거 (데이터 상황에 맞게 조정)
LOGRET_DROP_CENTER_Q    = 0.20          # 중앙 20%(|r| 작은 구간) 제거; 0이면 미사용
APPLY_FILTER_TO_HOLDOUT = True          # 논문식: holdout에도 동일 기준 적용 (실전은 False도 고려)
LOGRET_COL_CANDIDATES   = ["daily_logret", "logret_KOSPI", "log_ret", "ret_log"]

# === [ADD] 레이블 시프트 자동 판별(+1일 타깃 사용 여부) ===
def _quick_auc_for_shift(df: pd.DataFrame, shift_days: int = 0) -> float:
    """
    간단 AUC로 shift=0 vs shift=+1 비교(대략적인 지표). label_up만 사용.
    """
    ...
    if shift_days == 1:
        ...
        tmp["label_up"] = tmp["label_up_next"].astype(int)
    # 안전하게 간단한 시그널(예: 이전일 수익률의 부호)로 AUC 대충 측정
    sig = tmp["label_up"].shift(1).fillna(0).values  # 더미 시그널 (정교X, 비교용)
    y   = tmp["label_up"].values
    if len(np.unique(y)) < 2:
        return 0.5
    try:
        return float(roc_auc_score(y, sig))
    except Exception:
        return 0.5

def detect_label_shift(enhanced_csv: str) -> int:
    """
    shift=0(그대로) vs shift=+1 중 AUC가 더 높은 쪽 선택 → 1이면 +1일 시프트 사용.
    """
...
def _pick_ret_col(df: pd.DataFrame) -> str:
    ...
    없으면 close(or KOSPI_close)로 daily_logret을 생성하여 사용.
    """
...
def filter_by_log_return(
    df: pd.DataFrame, *, abs_tau: float = LOGRET_ABS_TAU, drop_center_q: float = LOGRET_DROP_CENTER_Q
) -> pd.DataFrame:
    """
    ...
    if drop_center_q and 0.0 < drop_center_q < 0.5:
        lo = r.abs().quantile(drop_center_q)
        hi = r.abs().quantile(1.0 - drop_center_q)
        mask &= ((r.abs() <= lo) | (r.abs() >= hi)) == False  # 중앙부 제거
        # 위 한 줄이 헷갈리면 다음 둘 중 택1:
        # mask &= ~((r.abs() > lo) & (r.abs() < hi))
...
def apply_logret_filter_pipeline(df_in: pd.DataFrame) -> pd.DataFrame:
    """
    (레이블 시프트 처리 이후) 보합 필터 적용. USE_LOGRET_FILTER=False면 통과.
    """
...
class WaveletAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
    # ── 빠른 경로: 훈련 중 SWT 생략 → CPU↔GPU 왕복 제거, 속도/안정성 향상 ──
        if FAST_WAVELET and self.training:
            B, T, C = x.shape
            # 한 스케일만 사용해도 충분: 0번 투영 + 멀티헤드 어텐션
            Q = self.to_q[0](x)
            K = self.to_k[0](x)
            V = self.to_v[0](x)
        ...
        # SWT는 CPU에서 동작하므로 텐서→넘파이→텐서 변환이 무겁다 (평가 시에만 수행)
        arr = x.detach().cpu().numpy()
        coeffs = pywt.swt(arr, self.wavelet, level=level, axis=1)  # [(cA, cD), ...]
        details = [torch.from_numpy(cD).to(x.device, dtype=x.dtype) for (_, cD) in coeffs]

        Vs = []
        for s in range(level):
            W = details[s]                     # (B, T(+pad), C)
            Q = self.to_q[s](W)                # (B, T, d_model)
            K = self.to_k[s](W)                # (B, T, d_model)
            V = self.to_v[s](W)                # (B, T, d_model)
            out, _ = self.attn(Q, K, V)        # (B, T, d_model)
            Vs.append(out)

        V_stack = torch.stack(Vs, dim=-1)      # (B, T, d_model, level)
        ...
        Z = (V_stack * gate.unsqueeze(1).unsqueeze(2)).sum(-1)  # (B, T, d_model)
...
class WaveAttTransformerClassifier(nn.Module):
    def __init__(
        ...
        """
        WaveletAttention + (옵션) Soft-DTW + Transformer Encoder + Classifier

        - USE_DTW = False: DTW 단계는 _DTWPassThrough로 대체되어 비용 거의 0
        - USE_DTW = True : 기존 DTWAttention(gamma=dtw_gamma) 사용
        """
        super().__init__()

        # 1) 입력 투영: (B, T, input_size) -> (B, T, d_model)
        self.input_proj = nn.Linear(input_size, d_model)

        # 2) 웨이블릿 어텐션: (B, T, C=input_size) -> (B, T, d_model)
        #    (훈련 중에는 FAST_WAVELET=True라면 WaveletAttention.forward에서 SWT를 생략하도록 구현되어 있어야 함)
        self.wav_att = WaveletAttention(input_size, d_model, nhead, n_scales)

        # 3) DTW 단계: 켜고 끄는 것을 전역 플래그로 제어 (메모리/시간 절약)
        #    - DTWAttention의 forward는 보통 (x_proj, wav_feat) 두 입력을 받도록 되어 있었음
        self.dtw_att = DTWAttention(gamma=dtw_gamma) if USE_DTW else _DTWPassThrough()

        # 4) 위치 인코딩 + 트랜스포머 인코더 스택
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # (2) 웨이블릿 어텐션
        z_wav = self.wav_att(x)                          # (B, T, d_model)

        # (3) DTW 또는 패스스루
        #     - USE_DTW=False이면 _DTWPassThrough가 z_wav를 그대로 반환
        z_dtw = self.dtw_att(x_proj, z_wav)              # (B, T, d_model)

        # (4) 위치 인코딩 + 인코더 스택
        h = self.pos_enc(z_dtw)                          # (B, T, d_model)
        for layer in self.enc_layers:
            h = layer(h)                                 # (B, T, d_model)

        # (5) 최종 정규화 + 마지막 타임스텝을 사용해 로지트 산출
        h = self.final_norm(h)                           # (B, T, d_model)
        logits = self.classifier(h[:, -1, :])            # (B, 1)
        return logits
...
def train_neural_model(model, train_loader, val_loader,
                       ...
                       early_stopping_patience=5, use_amp=True):
    ...
    best_auc, best_state, no_improve = -1e9, None, 0
    for epoch in range(1, epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            try:
                ...
                if "out of memory" in str(e).lower() and hasattr(train_loader, "batch_size"):
                    torch.cuda.empty_cache()
                    new_bs = max(8, train_loader.batch_size // 2)
                    logging.warning(f"[OOM] reduce batch_size {train_loader.batch_size} -> {new_bs}")
                    train_loader.batch_size = new_bs
                    continue
                raise
        scheduler.step()
        ...
        p = np.concatenate(probs).ravel(); y = np.concatenate(targs).ravel()
...
def evaluate_model(model, loader, margin: float = 0.0):
    """
    AUC/ACC/F1 + AUPRC(au-pr), Sensitivity(민감도), Specificity(특이도)까지 계산
    """
    model.eval()
    probs, targs = [], []
    with torch.no_grad():
    ...
    p = np.concatenate(probs).ravel()
    y = np.concatenate(targs).ravel()

    # (선택) flip-guard: AUC<0.5면 방향 뒤집기
    try:
        pre_auc = roc_auc_score(y, p) if len(np.unique(y)) > 1 else 0.5
    except Exception:
        pre_auc = 0.5
    if pre_auc < 0.5:
        p = 1.0 - p
    ...
    # 마진 기반 이진 지표
    met = bin_metrics_with_margin(y, p, margin=margin)

    # 이진 예측(0.5 기준)
    pred = (p > 0.5).astype(int)

    # 기본 지표
    auc_full = roc_auc_score(y, p) if len(np.unique(y)) > 1 else 0.5
    acc_full = accuracy_score(y, pred)
    f1_full  = f1_score(y, pred) if len(np.unique(y)) > 1 else 0.0
    ...
    sensitivity = recall_score(y, pred) if len(np.unique(y)) > 1 else 0.0
    try:
...
def retrain_and_save(study):
    """
    WaveAtt 최종 재학습 & 저장
    - (전처리) auto label shift → 보합 필터
    - (분할) 데이터 크기에 맞춘 안전 분할 + 80:20 fallback + seq_len 자동 보정
    - (학습) 마지막 분할로 학습, meta 구간에서 확률 보정자 학습
    - (저장) 모델/스케일러/보정자/파라미터 번들 저장
    반환: {"params": best_full, "saved_dir": str(final_model_dir)}
    """
    import json, logging
    import numpy as np
    import torch
    ...
    # 1) 레이블 시프트 자동 판별(+1일 타깃 적용) → 보합 필터
    df_all = load_dataset_auto_label(enhanced_csv)
    df_all = apply_logret_filter_pipeline(df_all)

    # 2) train/holdout 분리(holdout은 건드리지 않음)
    df, _ = _train_holdout_split(df_all)

    # 3) 베스트 하이퍼파라미터 복원
    best = study.best_trial.params if hasattr(study, "best_trial") else {}
    # 필수 키 복원(파일에 이미 있는 유틸 사용)
    ...
    seq_len       = int(best.get("seq_len", 84))
    transf_epochs = int(best.get("transf_epochs", 30))
    transf_lr     = float(best.get("transf_lr", 1e-3))
    transf_dropout= float(best.get("transf_dropout", 0.2))
    num_layers    = int(best.get("num_layers", 3))
    reject_margin = float(best.get("reject_margin", 0.0))

    # 4) 안전한 분할 구성
    n = len(df)
    MIN_TRAIN, MIN_VAL = 200, 60
    def _safe_last_split(n, seq_len, init_sugg, val_sugg):
        init = int(init_sugg); val = int(val_sugg)
        # 상하한 보정
        init = max(MIN_TRAIN, min(init, n - (MIN_VAL + seq_len + 1)))
        val  = max(MIN_VAL,   min(val,  n - (init + seq_len + 1)))
        splits = []
        te = init
        step = max(1, val // 2)
        while te + val <= n:
            tr_idx = list(range(te))
            va_idx = list(range(te, te + val))
            if len(va_idx) > seq_len:
                splits.append((tr_idx, va_idx))
            te += step
        if not splits:
            # 80:20 fallback
            cut = max(int(n * 0.8), MIN_TRAIN)
            cut = min(cut, n - max(MIN_VAL, seq_len + 1))
            cut = max(cut, 1)
            tr_idx = list(range(cut))
            va_idx = list(range(cut, n))
            if len(va_idx) <= seq_len:
                shrink = (seq_len - len(va_idx) + 1)
                seq_len = max(16, seq_len - shrink)
            splits = [(tr_idx, va_idx)]
        return splits, seq_len, init, val

    init_sugg = int(best.get("initial_train", 500 if n > 800 else max(200, n // 2)))
    val_sugg  = int(best.get("val_size", 120))
    splits, seq_len, initial_train, val_size = _safe_last_split(n, seq_len, init_sugg, val_sugg)
    tr_idx, va_idx = splits[-1]

    train_df = df.iloc[tr_idx].reset_index(drop=True)
    val_df   = df.iloc[va_idx].reset_index(drop=True)
    logging.info(f"[split] n={n} | seq_len={seq_len} | init={initial_train} | val={val_size} "
                 f"| final_window=({len(tr_idx)}, {len(va_idx)})")
    ...
    model, _ = train_neural_model(
        model, lb, lm,
        epochs=transf_epochs, lr=transf_lr, weight=weight
    )

    # 7) 메타(보정자) 학습
    calibr = ProbabilityCalibrator(dual_average=True); calibr.trained = False
    if len(meta_df) >= seq_len + 5:
        res_meta = evaluate_model(model, lm, margin=reject_margin)
    ...
    joblib.dump({
        ...
        "feat_cols": feat_cols,
        "calibrator": calibr,
        "params": {**best_full, "seq_len": seq_len, "reject_margin": reject_margin}
    ...
    logging.info(f"[retrain] saved bundle at: {final_model_dir}")
    return {"params": {**best_full, "seq_len": seq_len, "reject_margin": reject_margin},
...
class _DTWPassThroughExplain(nn.Module):
    """
    Explain 단계 전용 DTW 우회 모듈.
    DTWAttention 인터페이스와 동일하게 (x_proj, z_wav)를 입력으로 받고
    웨이블릿 출력(z_wav)을 그대로 반환해, wavelet 경로의 기울기가 보존되게 한다.
    """
    def __init__(self):
        super().__init__()
    def forward(self, x_proj: torch.Tensor, z_wav: torch.Tensor) -> torch.Tensor:
        # 중요: z_wav는 requires_grad=True 상태로 유지되어야 함
        return z_wav


class _DTWPassThrough(nn.Module):
    """
    DTWAttention을 끈 상태에서 쓰는 패스스루 블록:
    입력 시퀀스를 그대로 반환하여 추가 계산 비용이 0에 가깝다.
    """
    def __init__(self):
        super().__init__()
    def forward(self, x_proj: torch.Tensor, wav_feat: torch.Tensor) -> torch.Tensor:
        # 기존 DTWAttention 인터페이스와 동일한 시그니처를 유지(x_proj, wav_feat),
        # 하지만 실제로는 wav_feat(웨이블릿 어텐션 출력)을 그대로 반환한다.
        return wav_feat
...
def _grad_explain_model(
    ...
    """
    Grad 기반 기여도 시각화/저장.
    - disable_dtw_for_explain=True면 설명 동안 DTW를 우회해 wavelet 경로(z_wav)에 집중.
    - 종료 시 원래 dtw_att를 반드시 복구.
    """
    if X_seq.shape[0] == 0:
        return

    # 0) 샘플/배경 선택 및 베이스라인 구성
    ...
    # 2) DTW 임시 우회 적용(설명 전용)
    orig_dtw = None
    swapped = False
    if disable_dtw_for_explain and hasattr(model, "dtw_att"):
        try:
            orig_dtw = model.dtw_att
            model.dtw_att = _DTWPassThroughExplain().to(device)
            swapped = True
        except Exception as e:
            logging.warning(f"[explain] DTW swap skipped: {e}")

    try:
        # 3) Grad 기반 설명 (예: Integrated Gradients / Gradient * Input)
        xb.requires_grad_(True)
        logits = model(xb)                 # (B, 1)
        probs  = torch.sigmoid(logits)     # (B, 1)
        # 타깃은 양성 확률의 합(간단) — 필요시 특정 인덱스로 커스터마이즈
        target = probs.sum()
        model.zero_grad(set_to_none=True)
        target.backward()

        # 4) 기여도 스코어 산출: |grad * input| 평균을 특징 중요도로 사용(간단 버전)
        #    시퀀스 텐서 shape: (B, T, C)라고 가정 — 프로젝트 데이터셋 기준 확인 필요
        grad = xb.grad.detach()            # (B, T, C)
        contrib = (grad * xb).abs().mean(dim=(0, 1)).cpu().numpy()  # (C,)

        # 상위 topk 특징 추출
        idx = np.argsort(contrib)[::-1][:max(1, min(topk, len(contrib)))]
        names = None
        try:
            # 가능하면 피처 이름을 붙여준다.
            names = _get_feature_cols(pd.DataFrame(columns=range(contrib.shape[0])))
        except Exception:
            names = [f"f{i}" for i in range(contrib.shape[0])]
        top_names = [names[i] if i < len(names) else f"f{i}" for i in idx]
        top_vals  = contrib[idx]

        # 5) 저장: PNG 바 차트 + (옵션) CSV
        plt.figure(figsize=(8, max(3, int(0.35*len(top_names)))))
        plt.barh(range(len(top_names))[::-1], top_vals[::-1])
        plt.yticks(range(len(top_names))[::-1], top_names[::-1])
        plt.title("Gradient*Input Feature Importance (explain)")
        plt.tight_layout()
        ...
        if out_csv is not None:
            ...
            pd.DataFrame({"feature": top_names, "importance": top_vals}).to_csv(out_csv, index=False)

        logging.info(f"[explain] saved importance PNG: {out_png}{' and CSV: '+str(out_csv) if out_csv else ''}")

    finally:
        # 6) DTW 복구
        if swapped and (orig_dtw is not None):
...
def evaluate_on_holdout(saved):
    ...
    # [옵션] 홀드아웃에도 동일 보합 필터 적용 (레이블 기준 정합)
    ...
    # ── 번들 로드 (스케일러/특징/보정자/파라미터)
    bundle_path = Path(saved["saved_dir"]) / "preproc_and_calibrator.pkl"
    ...
    state_path = Path(saved["saved_dir"]) / "transformer_final.pt"
    ...
    # ── 예측
    _, p_all, _ = evaluate_model(trans, dl)  # 내부에서 flip-guard/추가지표 계산은 독립
    L  = min(len(p_all), len(y) - seq_len)
    vt = y[seq_len:seq_len + L]
    if len(vt) == 0:
        logging.warning("[HOLDOUT] not enough length for evaluation; returning neutral metrics.")
        return {"auc_overall": 0.5, "acc_overall": 0.0, "f1_overall": 0.0,
                "auprc": np.nan, "sensitivity": 0.0, "specificity": 0.0}
    ...
    # ── 추가 지표
    try:
        auprc = average_precision_score(vt, p_c)
    except Exception:
        auprc = np.nan
    sens = recall_score(vt, pred) if len(np.unique(vt)) > 1 else 0.0
    try:
        tn, fp, fn, tp = confusion_matrix(vt, pred).ravel()
    ...
    logging.info(
        ...
        f"auprc={auprc if np.isfinite(auprc) else float('nan'):.4f}, "
